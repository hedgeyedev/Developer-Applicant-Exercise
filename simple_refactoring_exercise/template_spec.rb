...
describe Template do
  ...
  it "should substitute %CODE% and %ALTCODE% in the template with multiple instances of the template vars" do
    expect(
      template('Code is %CODE%; alt code is %ALTCODE%, code is definitely not %ALTCODE%; alt code most certainly is not %CODE%', '5678901234')
  ...
  [rand(10 ** 10).to_s, rand(10 ** 10).to_s].each do |req_id|
    it "should have some fun with random numbers for req_id" do  
      expect(template('Code is %CODE%; alt code is %ALTCODE%', req_id)).to eq "Code is #{req_id}; alt code is #{req_id[0..4]}-#{req_id[5..7]}"
    end
  end
end
